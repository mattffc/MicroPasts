...
imBlurred = ndimage.gaussian_filter(im, 8)

sx0 = ndimage.sobel(imBlurred[:,:,0], axis=0, mode='constant')
sy0 = ndimage.sobel(imBlurred[:,:,0], axis=1, mode='constant')
#sob0 = np.hypot(sx0, sy0)
sx1 = ndimage.sobel(imBlurred[:,:,1], axis=0, mode='constant')
sy1 = ndimage.sobel(imBlurred[:,:,1], axis=1, mode='constant')
#sob1 = np.hypot(sx1, sy1)
sx2 = ndimage.sobel(imBlurred[:,:,2], axis=0, mode='constant')
sy2 = ndimage.sobel(imBlurred[:,:,2], axis=1, mode='constant')
